...
import { estadoCuentaPorDocumento } from '@/lib/activacion';
...
/** Enlace que acompaña a un aviso: decirle "ya tienes cuenta" sin una salida es un callejón. */
...
/**
 * Alta de un alumno nuevo. Es el formulario de siempre; lo único que cambió es
 * que ahora, antes de intentar el alta, pregunta si ese documento ya tiene
 * cuenta — el caso del cliente de certificación directa, cuya cuenta existe
 * desde que le emitimos el certificado. Ese no se registra: activa.
 */
export default function FormNuevaCuenta({ onIrAActivar }: { onIrAActivar: (documento: string) => void }) {
  ...
  const [aviso, setAviso] = useState<{ texto: string; tipo: 'ok' | 'err'; accion?: AccionAviso } | null>(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Un documento ya registrado choca contra `perfiles_documento_unique` dentro
    // del trigger que crea el perfil, y el trigger solo resuelve el conflicto
    // por `id`: Supabase devuelve un "Database error saving new user" que no le
    // dice nada a nadie. Se pregunta antes y se manda a la pestaña correcta.
    setCargando(true);
    const estado = await estadoCuentaPorDocumento(doc);
    setCargando(false);
    if (estado !== 'sin_cuenta') {
      const wsp = whatsappLink(
        WHATSAPP_PEDIDOS,
        `Hola, quiero activar mi cuenta del aula virtual. Mi documento es ${doc} y necesito un código de acceso.`
      );
      const mensajes: Record<typeof estado, { texto: string; accion: AccionAviso }> = {
        pendiente: {
          texto: 'Ya tienes una cuenta creada con ese documento, de cuando te certificaste con nosotros. No la vuelvas a crear: actívala con el código que vino junto a tu certificado.',
          accion: { texto: 'Ir a activar mi cuenta', onClick: () => onIrAActivar(doc) },
        },
        sin_codigo: {
          texto: 'Ya tienes una cuenta creada con ese documento, pero todavía no tiene código de acceso.',
          accion: { texto: 'Pedir mi código por WhatsApp', href: wsp },
        },
        bloqueada: {
          texto: 'La activación de esa cuenta quedó bloqueada por seguridad tras varios intentos fallidos.',
          accion: { texto: 'Pedir un código nuevo por WhatsApp', href: wsp },
        },
        activa: {
          texto: 'Ese documento ya tiene una cuenta activa. Inicia sesión con tu correo y contraseña.',
          accion: { texto: 'Iniciar sesión', href: '/login' },
        },
      };
      const m = mensajes[estado];
      setAviso({ texto: m.texto, tipo: 'err', accion: m.accion });
      return;
    }
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          ...
          // Quien llega hasta acá eligió su correo y su contraseña por su
          // cuenta: nace activada. Sin esta marca, el trigger la dejaría "sin
          // estrenar" y más adelante el sistema le ofrecería activar una cuenta
          // que ya es suya — ver migración 006.
          auto_registro: true,
        },
      },
    });
      ...
      {aviso && (
        <div className={`aviso ${aviso.tipo}`}>
          ...
          {aviso.accion?.onClick && (
            <button type="button" className="enlace-accion" onClick={aviso.accion.onClick}>
              {aviso.accion.texto}
            </button>
          )}
          {aviso.accion?.href && (
            <a href={aviso.accion.href} target={aviso.accion.href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer">
              {aviso.accion.texto}
            </a>
          )}
        </div>
